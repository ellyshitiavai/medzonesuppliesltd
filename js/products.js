...
// GitHub repo info
const repoOwner = "ellyshitiavai";
const repoName = "medzonesupplyltd";
const folderPath = "content/products"; // ✅ Only one content
...
async function loadProducts() {
  try {
    ...
    productList.innerHTML = products.map((p, index) => {
      ...
      return `
        <div class="product-card product-fade-in" style="animation-delay:${index * 0.1}s; border:1px solid #ccc; padding:10px; border-radius:8px; text-align:center;">
          <img src="${p.image || 'placeholder.png'}" alt="${p.title}" loading="lazy" style="max-width:100%; border-radius:8px;">
          <h4>${p.title}</h4>
          <p>${p.description || ''}</p>
          <strong>${p.price || ''}</strong>
          <div style="margin-top:10px; display:flex; justify-content:center; gap:10px;">
